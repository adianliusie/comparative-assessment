...
for system in flant5-base flant5-large flant5-xl flant5-xxl llama2-7b-chat llama2-13b-chat; do
    for i in 1 2; do
	python ../system_run.py --dataset $dataset --score-type $score --output-path output_text/$dataset/$system/$score/scoring-$i --system $system --prompt-id s$i --shuffle --device $device --max-len 3
